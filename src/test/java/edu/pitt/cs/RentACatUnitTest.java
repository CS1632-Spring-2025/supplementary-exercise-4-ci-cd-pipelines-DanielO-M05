...
public class RentACatUnitTest {
	...
	public void testGetCatNumCats3() {
		...
		try {
			...
			assertNotNull(ret);
			assertTrue(ret instanceof Cat);
        	Cat cat = (Cat) ret; // Want to cast to check ID
        	assertEquals(2, cat.getId());
		} catch (Exception e) {
			e.printStackTrace();
			fail("Whomp whomp" + e.getMessage());
		}
	}
	...
	public void testRenameFailureNumCats0() {
		...
		Mockito.verify(c2, never()).renameCat("Garfield"); // Perhaps a little pedantic, but the cat can be renamed anything but "Garfield"
		assertEquals("Invalid cat ID." + newline, out.toString());
	}
	...
	public void testRentCatFailureNumCats3() {
		...
		assertFalse(ret);
		Mockito.verify(c2, never()).rentCat(); // Cat should not have been rented
